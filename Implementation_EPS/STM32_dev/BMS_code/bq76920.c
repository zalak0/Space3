...
{
    return HAL_I2C_IsDeviceReady(&hi2c1, BQ76920_I2C_ADDR, 3, 100) == HAL_OK;
}
...
bool BQ76920_ReadCalibration(BQ76920_Calibration *cal)
{
    ...
    adc_gain_bits = ((gain1 & 0x0C) << 1) | ((gain2 & 0xE0) >> 5);
...
static float BQ76920_AdcToCellVoltage(uint16_t adc)
{
    ...
    mv = ((float)adc * (float)g_cal.adc_gain_uv_per_lsb) / 1000.0f;
    mv += (float)g_cal.adc_offset_mv;
...
bool BQ76920_ReadCellVoltage(uint8_t cell_index, float *voltage_v)
{
    ...
    adc = BQ76920_Combine14Bit(data[0], data[1]);
